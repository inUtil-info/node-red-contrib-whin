...
/// lu4t
  ...
  function WhinConfirm(config){
    ...
    const key = node.authconf.token.slice(8,20)	  
    //const key = "IN"+phone;
    var toolate = false, alreadysent=false;
    var request="";
		const options = {
				hostname: 'whin.inutil.info',
				port: 443,
        ...
        req.write(postData);            
        node.status({fill:"green",shape:"dot",text:"Awaiting Response"});    
        req.end()	;
        toolate=false; alreadysent=false;
        delay = msg.ttl;
        const myTimeout = setTimeout(timeout, msg.ttl);
        function timeout(){
          toolate=true;
          var nm={};
          nm.payload="TimeOut";
    ...
    RED.nodes.registerType("whin-confirm", WhinConfirm);
}
